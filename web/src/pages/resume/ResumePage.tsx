        ...
        <section className="p-4 lg:p-12 print:p-0 print:pt-2">
          ...
          <ResumeSection title="Work Experience">
            <div className="resume-timeline pl-8">
              {WorkHistory.sort((a, b) => (a.from < b.from ? 1 : -1))
                .slice(0, 5)
                .map((item, i) => {
                  const fromDate = format(new Date(item.from), "yyyy");
                  const toDate = !item.to
                    ? "Present"
                    : format(new Date(item.to), "yyyy");
                  return (
                    <ResumeHistory
                      key={i}
                      title={item.title}
          ...
          <ResumeSection title="Education">
            <div className="grid grid-cols-1 lg:grid-cols-3 print:grid-cols-3 gap-2">
              {Education.map((item, i) => (
                <div
                  key={i}
                  className="mb-2 flex flex-no-wrap space-x-1 lg:flex-col lg:space-x-0 print:flex-col print:space-x-0"
                >
                  <div className="font-bold whitespace-no-wrap truncate">
                    {item.name}
                  </div>
                  <div className="font-light whitespace-no-wrap truncate text-sm print:text-xs">
                    {item.provider}
                  </div>
                  <div className="font-light whitespace-no-wrap truncate text-sm print:text-xs">
                    {item.completed}
